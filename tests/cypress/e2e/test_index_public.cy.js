...
describe("Index page when awdeorio is logged in with real database", () => {
  ...
  it("Shows a new comment without a page refresh on normal server", () => {
    ...
    cy.get("form.comment-form input[type='text']")
  ...
  it("Removes a comment from the UI after pressing the delete comment button on normal server", () => {
    ...
    cy.get(".comment-text:contains(#chickensofinstagram)").should(
      "have.length",
      2
    );
    ...
    cy.get("button.delete-comment-button")
      .should("have.length", 3)
      .eq(0)
      .click();
    ...
    cy.get("button.delete-comment-button").should("have.length", 2);
  });
  ...
  it("Updates the UI when liking or unliking a post on normal server", () => {
    ...
    cy.get("button.like-unlike-button").should("have.length", 3).eq(0).click();
    ...
    cy.get("button.like-unlike-button").eq(0).click();
